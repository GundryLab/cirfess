...
###########################################################
# remove records from the database of new/updated sequences
###########################################################

for ID in seqs_dict:
    protSQL = "DELETE FROM prot Where Accession = '" + ID + "'" 
    pepSQL = "DELETE FROM pep Where Accession = '" + ID + "'" 
    try:
        cursor.execute(protSQL)
        cursor.execute(pepSQL)
